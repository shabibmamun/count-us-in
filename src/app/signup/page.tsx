...
export default function SignUpPage() {
  const { signUp, logInWithGoogle, isFallbackMode } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSubmitted(true);
      if (isFallbackMode) {
        // Fallback mode directly logs in and goes to onboarding
        router.push('/onboarding');
      }
    } catch (err: any) {
      setError(err.message || 'We could not create your space. Check your connection.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Google sign up flows directly into onboarding if new
      router.push('/onboarding');
    } catch (err: any) {
    ...
    <div className="min-h-screen flex flex-col justify-center bg-background py-12 px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 className="mt-6 text-3xl font-extrabold text-primary">
          Create your Count Us In space
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Build one clear view of your money.
        </p>
      </div>
          ...
          {isSubmitted && !isFallbackMode ? (
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                We sent a secure sign-in link to <span className="font-medium text-foreground">{email}</span>. Click the link to set up your profile and currency.
              </p>
            </div>
          ) : (
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                <label htmlFor="pass" className="block text-sm font-medium text-primary mb-1">
                  Password (optional)
                </label>
                <input
                  id="pass"
                ...
                <span className="text-xs text-muted-foreground mt-1 block">
                  Password can be set for traditional login, or sign in via email link.
                </span>
              </div>
              ...
              <div>
                ...
                >
                  {isLoading ? 'Creating space...' : 'Create your space'}
                </button>
              </div>
